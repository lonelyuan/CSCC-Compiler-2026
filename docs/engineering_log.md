...
## 2026-08-05 换到鲲鹏级 aarch64 平台复测；chunk 预算 50000 → 200000（Round 15）

本轮把测量搬到一台与评测机同 ISA / 同 OS / 同工具链的机器上,然后检验 Round 8–14 那批常量
在这台机器上还成不成立。结论是:**聚合分数几乎没变,但分数的来源变了,而且 Round 14 最后
一次调参在这台机器上是反向的**——该常量已按目标平台改回 200000 并全量验证。

### 新测量平台（首次接近官方性能平台）

之前所有有效数据都来自 40 物理核 x86_64 Xeon 调试机,与官方平台既不同 ISA 也不同工具链。
新平台通过 tailscale 隧道接入的云桌面容器:

```text
HiSilicon aarch64 (SVE/i8mm/bf16), openEuler 22.03 LTS, kernel 5.10
40 核 / 单 NUMA 节点 / 2.9 GHz 固定(boost disabled) / L3 32 MiB
BiSheng Enterprise 3.2.0.1.B004 clang 15.0.4  —— 与旧 VM 记录的版本完全一致
```

入口脚本:`scripts/cg.sh`(ssh 多路复用)、`scripts/sync_to_cg.sh`、`scripts/cg_run.sh`
(detach + 轮询,链路掉线不丢作业——本轮实测掉过一次)。主机地址放在未跟踪的
`scripts/cg_host.env`,与 xeon 一致。

**踩坑:容器有 2 GiB 内存 cgroup 硬上限**(`memory.limit_in_bytes` 只读,`free` 显示的 77 GB
是宿主视图)。`main_percase.cpp` 按官方 `matrix_case_io` 约定把 150 个用例的输入**和**输出全部
驻留,合计 2.46 GB,于是整套 150 用例在 `[4/6]` 阶段被 OOM killer 打掉(exit 137)。
新增 `scripts/percase_bench_chunked.sh`:按用例声明尺寸把 spec 切成 ≤600 MB 的 5 段分别跑,
再用 `scripts/merge_chunk_csvs.py` 还原全局 case index 后统一按判题口径打分。
per-case 计时是按单次 `block_cholesky` 调用测的,所以切段不改变任何一个用例的数字;
唯一变化是一次性进程开销从"整轮付一次"变成"每段付一次",而 Round 13 已经把预热移出计时区。

### 基线:aarch64 上 geomean 4.253x，总分 47.97（150/150 PASS）

`PASSES=3 REPEAT=1`,每用例取 3 遍最小值,两侧全过 verifier。
证据 `docs/benchmark_results/r15_cg_aarch64_baseline.csv`。

和 Xeon 的 Round 13/14 放在一起看,**聚合数字落在同一个噪声带内**(Xeon 两遍 4.632x/4.227x,
本机 4.253x),但分桶结构明显换了位置:

| 桶 | 用例数 | Xeon | aarch64 | arm/xeon |
| --- | ---: | ---: | ---: | ---: |
| `b < 12` | 22 | 2.10x | 1.94x | **0.92** |
| `12 <= b < 32` | 39 | 4.22x | 3.60x | **0.85** |
| `32 <= b < 128` | 61 | 6.11x | 6.56x | 1.07 |
| `b >= 128` | 28 | 3.64x | 3.87x | 1.06 |

**小 tile 变差、大 tile 变好**,而不是整体等比缩放。

### 机制：串行基准在这台机器上快了 1.8 倍，于是"每 task 多少 flops"整条标定失效

官方 `madd` 是朴素三重循环,但 `A[row][k] * B[col][k]` 两边在 k 上都连续,向量化很好。
这台机器 2.9 GHz 定频、无 AVX 降频,实测**串行基准时间只有 Xeon 的 0.55 倍**
(`n>=768` 的 80 个用例中位数 0.560;按桶 0.65 / 0.61 / 0.53 / 0.49)。

Round 14 把 range chunk 预算从 200000 调到 50000 flops,依据是"每个 task 约 5µs 工作量"——
**但 5µs 是 Xeon 的 madd 吞吐换算出来的**。同样 50000 flops 在这台机器上只有约 2.75µs,
低于最优工作量,于是队列往返摊不平。这正好落在变差的那两个桶上。

扫参证明了这一点(`n=1152`,cap 默认,repeat=3,每点过 verifier,
`docs/benchmark_results/r15_cg_range_flops_sweep.csv`):

| b | 25000 | **50000(当前默认)** | 100000 | **200000** | 400000 |
| --- | ---: | ---: | ---: | ---: | ---: |
...
**这台机器的最优值是 200000,恰好是 Round 14 调离的那个值。** 相对当前默认:
`b=24` +27.9%、`b=32` +10.7%、`b=16` +10.0%、`b=12` +6.2%、`b=8` +1.8%,
`b>=64` 持平(chunk 已经是 1,预算不起作用)。

注意**幅度比模型预测的大**:纯按吞吐换算应该是 50000/0.55 ≈ 90000,实测最优在 200000。
说明除了"每 task 工作量变小",这台机器的**每 task 队列开销本身也更贵**(aarch64 原子/futex
路径与 x86 不同),两个方向叠加。所以不能只做吞吐折算,必须实测。

### 修正一条写在代码注释里的错误假设

`participantCapForTile` 和 `asyncMinBlockSize` 的注释都写着同一句话:

> aarch64 has narrower vector units, so each tile task runs longer and the
> sustainable count there is at least as high -- the direction of the error is safe.

**这句话的前提是错的。** 这台 aarch64 上同样的 tile 跑得**更快**(0.55 倍时间),不是更慢,
所以 task 更短、每 task 开销占比更高,"误差方向是安全的"这个结论不成立——小 tile 上的
0.85/0.92 就是代价。注释需要按实测改写,不能继续作为"不用复测"的理由。

### 反直觉：8 个跑不过串行的用例只值 0.14 分

分布里最扎眼的是 8 个 <0.95x 的用例(`n=320 b=8` **0.490x**、`n=192 b=16` 0.606x、
`n=384 b=8` 0.614x、`n=320 b=10` 0.700x、`n=448 b=8` 0.760x)。它们确实是
`resolveThreadCount` 里那两个 Xeon 交叉点(`n<192`、`b<12 && n<320`)在这台机器上偏低导致的:
串行快了 1.8 倍,交叉点应该整体上移。

但按判题口径算,把这 8 个全部救到 1.00x 只有 **+0.14 分**(4.253x → 4.330x)。
而把两个小 tile 桶抬到大 tile 桶那样的 arm/xeon 比例值 **+0.68 分**。
**等权重几何平均下,"很难看的用例"和"很值钱的用例"不是同一批**——8 个 0.5x 的用例和
39 个少 15% 的用例,后者贵 5 倍。

### 已验证并采纳：chunk 预算 50000 → 200000（全量 150 用例）

`PASSES=3 REPEAT=1`,150/150 verifier PASS,证据 `docs/benchmark_results/r15_cg_flops200k.csv`:

| | geomean | 总分 |
| --- | ---: | ---: |
| 50000(Round 14 默认) | 4.253x | 47.97 |
| **200000(本轮采纳)** | **4.370x** | **48.19** |

聚合只 +2.7%,**落在本机 5.7–9.6% 的运行间噪声带内**,所以不能只看这个数字。
按 Round 13/14 定下的方法做配对分组对照——判据是"这个改动**能不能**影响该用例",
即两档预算算出的 chunk 长度是否不同(分界正好是 `b<=36` 对 `b>=40`):

| 分组 | 用例数 | 50000 | 200000 | 变化 |
| --- | ---: | ---: | ---: | ---: |
| **受影响**(chunk 长度不同) | 80 | 3.383x | 3.553x | **+5.0%** |
| **对照**(两档都是 chunk=1) | 70 | 5.524x | 5.535x | +0.2% |

**对照组 +0.2% 说明这不是主机漂移**,而受影响组 +5.0% 且**随 b 单调**:
`b=18` +13.5%、`b=24` +23.9%、`b=28` +16.2%、`b=16` +6.9%、`b=14` +5.8%、`b=12` +3.9%。
唯一反向是 `b=8` 组的 −2.5%(17 个用例),落在它自己 ±10% 的代码布局噪声内,
且与单用例扫参(2.79x → 2.84x)矛盾,判为噪声。

分桶变化正是预测的位置:`12<=b<32` **3.60x → 3.97x(+10.3%)**,
`b>=128` 3.87x → 3.91x(没有吃掉收益,此前担心的 0.6% 反向在全量上不成立),
`32<=b<128` 6.56x → 6.59x,`b<12` 1.94x → 1.91x。

**这个常量因此在同一份代码里被反向调了两次,两次都有完整证据链——因为它标定的是每 task
的微秒数,却写成了每 task 的 flops。** 现在按目标平台(aarch64)取值,Xeon 反而成了偏离
最优的那台。

### 下一个已定位但未采纳的杠杆：参与者上限 24 在新工作点上也错了

24 是在 50000 预算下拟合的。换到 200000 后重扫线程数(cap off,`n=1152`,repeat=3,
每点过 verifier,`docs/benchmark_results/r15_cg_participant_sweep_200k.csv`):

| b | T=8 | T=16 | T=24 | T=32 | T=40 | 最优 | cap=24 的代价 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
...
| 64 | 6.65x | 10.91x | 13.55x | 15.30x | **16.24x** | 40 | (未受限) |

**上限同时"对 b=8 太高、对 b>=16 太低"**:b=8 想要 8(24 亏 7%),b=32 想要 32(亏 7.4%)。
一条单调规则(b=8→8、b=12→16、`16<=b<48`→32、其余不限)能同时拟合这 6 个点,
但它只来自一个矩阵尺寸,而上限与 block 数有交互,**必须先扫 n 再做全量验证才能改默认值**。
若成立,估计值 ~+5% × 约 50 个 `12<=b<48` 用例 ≈ **+0.13 分**。

### 常量重标定不足以换档，这是本轮最重要的战略结论

本轮把目标平台上最大的一个常量错误修好了,收益是 **+0.22 分**(47.97 → 48.19);
下一个已定位的杠杆估计 +0.13 分;把 8 个跑不过串行的用例全救活是 +0.14 分。
而 `score 50 需要 geomean 5.33x`,是当前的 **1.22 倍**。

**也就是说:把所有已知的平台常量都调对,合起来也只有 ~0.5 分,离换档差一个数量级。**
真正的空间仍然在结构上(跨 panel 依赖恢复、把两次相位屏障拿掉),
不在继续拟合旋钮。常量重标定的价值是**让结构性改动在正确的工作点上被评估**——
在错误的 chunk 预算下测一个新调度策略,很可能得到反向的结论,这才是本轮最实际的收益。

### 经验

- **在目标 ISA 上复测之前,任何"每 task 多少工作量"的常量都只是本机常量。** 本轮最贵的一课:
  Round 14 那次调参在 Xeon 上有 6 个用例同向改善、证据链完整,依然是平台专属的——因为它标定
  的物理量(每 task 微秒数)被写成了一个平台相关的代理量(每 task flops)。
  **旋钮应该尽量用目标物理量表达,或者在运行时自己测。**
- **注释里的"误差方向是安全的"必须有测量支撑,否则它只是把复测无限推迟的借口。** 这次那句话
  不仅没支撑,前提(aarch64 更慢)还正好是反的。
- **等权重几何平均下要按"log 贡献"排优先级,不能按"speedup 多难看"排。** 8 个 0.5x 值 0.14 分,
  39 个 -15% 值 0.68 分。
- **测量宿主的资源限制要先查再跑。** 2 GiB cgroup 上限藏在 `free` 显示的 77 GB 后面,
  代价是一次跑到 [4/6] 才被 OOM 打掉的全量。
- **"配对分组对照"这次真正救了一个改动。** 聚合 +2.7% 在噪声带内,单看它既不能采纳也不能否证;
  按"能不能被影响"分成 80/70 两组后,对照组 +0.2%、受影响组 +5.0% 且随 b 单调,结论立刻确定。
  **分组判据要用机制(chunk 长度是否改变),不要用直觉(b 大不大)。**
- **同一个常量被反向调两次,不是调参失误,是量纲选错了。** 两次都有完整证据链、都在各自平台
  上正确。真正的教训是常量的**表达方式**:写成 flops 就必然平台相关,写成微秒才是它想标定的
  东西。下一步应该考虑让运行时在初始化时实测一次 madd 吞吐,把预算从 flops 换成时间。

### 补充：把"当前设计的结构上限"算出来之后，路线判断被推翻

上一节说"真正的空间在结构上(跨 panel 依赖恢复、去掉两次相位屏障)"。**把上限算清楚后
这句话是错的**,在此更正。

上限模型(相位屏障,零开销、相位内完美并行,核数 C=40):以 `b^3` 为单位,
`cholesky=1/3`、`trsm=1`、`madd=2`,每个 panel 跨度
`= 1/3 + ceil(ntrsm/C)*1 + ceil(nmadd/C)*2`。

| 桶 | 用例数 | 当前 | 本设计上限 | 达成率 |
| --- | ---: | ---: | ---: | ---: |
| `b < 12` | 22 | 1.91x | 36.6x | **5.2%** |
| `12 <= b < 32` | 39 | 3.97x | 30.6x | **13.0%** |
| `32 <= b < 128` | 61 | 6.59x | 12.9x | 51.3% |
| `b >= 128` | 28 | 3.91x | 4.1x | **96.2%** |

**做到自身上限的 80% 就是 geomean 12.11x / 总分 62.71,而第一名 63 分反解出来是 12.27x。**
所以:**不动相位屏障也能到第一名的水平**,历史上三次否证跨 panel DAG 不是实现没做好,
而是它本来就不在关键路径上。

模型的可信度由 `b >= 128` 桶给出:那一桶达成率 96.2%,说明模型在结构受限时是紧的,
不是一个虚高的理论值。

同时 `b >= 128` 这 28 个用例**已经没有空间了**,继续在大 tile 上投入是浪费。

机制线索已经在手:参与者扫参显示 `b=8` **加核心是负收益**(T=8 3.34x → T=40 2.76x),
`b=12` 在 16 核饱和。粒度不足只会让曲线平掉,**掉头向下是争用的特征**。
`b=8` 的算术强度只有 0.67 flop/byte,先验上最可能是内存带宽饱和;
但也可能是共享 ready queue 的锁争用(Round 10/11 当年是用"限制参与者数"绕开的,没有解决)。
