...
`smoke_test.sh` 和 `benchmark.sh` 都会透传 `COMPILER2026_DAG_THREADS`、`COMPILER2026_DAG_PROFILE`、`COMPILER2026_TASK_BATCH`、`COMPILER2026_ASYNC_MIN_B`。因此小范围 verifier smoke 可以直接验证 async 阈值、线程数、profile 和 batch 覆盖是否真实生效。
