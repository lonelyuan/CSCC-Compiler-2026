...
> **2026-08-05 Round 25 起,先读本节,再读 Round 15 和其余历史。** Round 15 的结构上限
> 仍是正确的零开销上界,但“做到上限 80%”不能再当作已证明可达:无锁执行和二维复用落地后,
> `b=8` 已在 32 核进入带宽平台,小矩阵则被每 panel 固定延迟主导。相位屏障仍不是首要结构瓶颈,
> 但当前 panel-local 设计在这台机器上的**实测**结果是 7.006x / 53.14 分,不是 12.11x。

## 当前最高优先级：跨过带宽与固定延迟，而不是继续换队列/屏障（Round 17–25）
...
`docs/benchmark_results/r20_cg_tile_group_default.csv`（以及 `_c1`–`_c5`）:
...
| --- | ---: | ---: | ---: | ---: |
| `b < 12` | 22 | 6.61x | 36.6x | 18.1% |
| `12 <= b < 32` | 39 | 9.07x | 30.6x | 29.6% |
| `32 <= b < 128` | 61 | 7.98x | 12.9x | 61.9% |
| `b >= 128` | 28 | 3.86x | 4.1x | 94.1% |

二维 tiling 的配对机制对照成立:第一段 98 个用例中，能被影响的 `b<28` 41 例是
**1.3067x**，控制组 `b>=28` 57 例是 **0.9987x**。它不是主机漂移，但只覆盖 41/150，
不足以独自达到 60 分；当前还需要全量 **1.522x**。
...
显著减少每 panel 固定工作，或同时覆盖 `12<=b<128` 大量用例的改动，才可能提供 1.522x。
